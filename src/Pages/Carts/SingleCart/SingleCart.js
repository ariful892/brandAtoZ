...
import { getStoredCart, removeFromDb } from '../../../utilites/cartStorage';
        ...
        <div className="single-cart-container shadow-lg ">
            <div className='cart-info'>
                ...
                <div className="ml-3 lg:ml-5 md:ml-5">
                    <h2 className='text-lg font-bold mb-.5 w-48 lg:w-full md:w-full'>{cart?.cart?.name}</h2>
                    {/* <p className='s-products-type'>{product.category}</p> */}

                    <p className='text-sm '>Color: {cart?.cart?.clr}</p>
                    <p className='text-sm '>Size: {cart?.cart?.sz}</p>

                    <p className='text-sm '>Quantity: {cart?.cart?.qty}</p>

                    <p className='text-sm mb-1'>Price: {cart?.cart?.price}</p>
            ...
            <div className='mt-1 mr-1'>
                {/* < onClick={removeProduct} className='w-5 h-5 lg:w-8 lg:h-6 text-red icon'  /> */}
                <h1 onClick={removeProduct} className='text-red-700'>Delete</h1>
